...
unsigned int nStakeMinAge = 1 * 60; // 1 min
...
const string strMessageMagic = "Stake Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    } else if (nHeight > 1 && nHeight < 43000){
      nSubsidy = 777 * COIN;
    } else {
      nSubsidy = 0 * COIN;
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nBestHeight >= 43000 && nBestHeight <= 77777){
      nSubsidy = 777 * COIN;
    } else {
      nSubsidy = 77 * COIN;
    }
