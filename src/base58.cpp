...
namespace {
    class CStakeAddressVisitor : public boost::static_visitor<bool> {
    private:
        CStakeAddress *addr;
    public:
        CStakeAddressVisitor(CStakeAddress *addrIn) : addr(addrIn) { }
...
bool CStakeAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CStakeAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CStakeAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CStakeAddressVisitor(this), dest);
}

bool CStakeAddress::IsValid() const {
...
CTxDestination CStakeAddress::Get() const {
    if (!IsValid())
...
bool CStakeAddress::GetKeyID(CKeyID &keyID) const {
...
bool CStakeAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CStakeSecret::SetKey(const CKey& vchSecret) {
...
CKey CStakeSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CStakeSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CStakeSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CStakeSecret::SetString(const std::string& strSecret) {
