             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CStakeAddress(strAddress).Get()];
        }
        else if (strType == "tx")
        {
            uint256 hash;
        ...
        else if (strType == "keymeta")
        {
            ...
            if (!pwallet->nTimeFirSTAKEey ||
                (keyMeta.nCreateTime < pwallet->nTimeFirSTAKEey))
                pwallet->nTimeFirSTAKEey = keyMeta.nCreateTime;
        }
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    // nTimeFirSTAKEey is only reliable if all keys have metadata
    if ((wss.nKeys + wss.nCKeys) != wss.nKeyMeta)
        pwallet->nTimeFirSTAKEey = 1; // 0 would be considered 'no value'
