...
//
// Bootup the masternode, look for a 25000 STAKE input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Stake.conf */
...
{
    CStakeAddress address(collateralAddress);
