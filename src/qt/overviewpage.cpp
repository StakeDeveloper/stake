...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeStakeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStakeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeStakeAmount = strAnonymizeStakeAmount.remove(strAnonymizeStakeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStakeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeStakeAmount*COIN) nMaxToAnonymize = nAnonymizeStakeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeStakeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeStakeAmount));
        strAnonymizeStakeAmount = strAnonymizeStakeAmount.remove(strAnonymizeStakeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStakeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
