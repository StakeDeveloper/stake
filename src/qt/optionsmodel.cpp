...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStakeAmount"))
        settings.setValue("nAnonymizeStakeAmount", 1000);
    nAnonymizeStakeAmount = settings.value("nAnonymizeStakeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeStakeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeStakeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeStakeAmount:
            return QVariant(nAnonymizeStakeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeStakeAmount:
            nAnonymizeStakeAmount = value.toInt();
            settings.setValue("nAnonymizeStakeAmount", nAnonymizeStakeAmount);
            emit AnonymizeStakeAmountChanged(nAnonymizeStakeAmount);
            break;
        default:
            break;
        }
